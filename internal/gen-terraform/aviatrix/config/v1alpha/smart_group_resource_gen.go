...
// generatedFile: aviatrix/config/v1alpha/smart_group_resource_gen.go
//
...
type SmartGroupClientGetter interface {
	SmartGroupClient() configpb.SmartGroupServiceClient
}
...
// SmartGroupResource defines the resource implementation.
type SmartGroupResource struct {
	client    configpb.SmartGroupServiceClient
...
func (r *SmartGroupResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	clients, ok := req.ProviderData.(SmartGroupClientGetter)
	if !ok {
		resp.Diagnostics.AddError(
			"unexpected ProviderData type",
			fmt.Sprintf("Expected SmartGroupClientGetter, got: %T. Please report this issue to the provider developers.", req.ProviderData),
		)
		return
	}

	r.client = clients.SmartGroupClient()
...
func (r *SmartGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// Call the API
	result, err := r.client.CreateSmartGroup(ctx, apiReq)
	if err != nil {
...
func (r *SmartGroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	// Call the API
	result, err := r.client.GetSmartGroup(ctx, &configpb.GetSmartGroupRequest{
		Name: data.Name.ValueString(),
	})
	if err != nil {
		status, ok := status.FromError(err)
		if ok && status.Code() == codes.NotFound {
			tflog.Info(ctx, fmt.Sprintf("SmartGroup %s not found, removing from state", data.Name.ValueString()))
			resp.State.RemoveResource(ctx)
			return
		}
		resp.Diagnostics.AddError(fmt.Sprintf("failed to read smart_group %s", data.Name.ValueString()), err.Error())
		return
	}
...
func (r *SmartGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Call the API
	result, err := r.client.UpdateSmartGroup(ctx, updateReq)
	if err != nil {
...
func (r *SmartGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	deleteReq := &configpb.DeleteSmartGroupRequest{
		Name: data.Name.ValueString(),
	}
	...
	// Call the API
	_, err := r.client.DeleteSmartGroup(ctx, deleteReq)
	if err != nil {
